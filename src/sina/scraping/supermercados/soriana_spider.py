...
def scrape_soriana(config: BrowserConfig = BrowserConfig()) -> List[Dict[str, Any]]:
    ...
    """
    log.info("Iniciando Spider Soriana...")

    repo_catalogo = CatalogoRepository()
    rutas_activas = repo_catalogo.obtener_rutas_activas(tienda="Soriana")

    if not rutas_activas:
        log.warning("No hay rutas activas en el catálogo.")
        return []

    log.info("Encontradas %d rutas activas", len(rutas_activas))
    ...
    with sync_playwright() as p:
        ...
        try:
            for route in rutas_activas:
                log.info("%s > %s", route['departamento'], route['categoria'])
            ...
            log.info("EXTRACCIÓN FINALIZADA: %d productos", len(productos_extraidos))

        except Exception as e:
            log.error("Error en spider Soriana: %s", e)
        finally:
            browser.close()
...
def main() -> None:
    ...
    log.info("Ejecutando scraper Soriana...")
    productos = scrape_soriana()

    log.info("Total productos: %d", len(productos))
    for p in productos[:5]:
        log.info("   - %s ($%s) - %s > %s", p['producto'], p['precio'], p['departamento'], p['categoria'])
